...
public class Algorithm {
    ...
    public static StackedPolytope calculateStackedPolytope1(ApollonianNetwork aN) {
        ...
        for (int i = 0; i < 3; i++) {
        	xD = (points[i].x().multiply(pert).setScale(0,BigDecimal.ROUND_FLOOR)).divide(pert,prec,BigDecimal.ROUND_HALF_UP);
            yD = (points[i].y().multiply(pert).setScale(0,BigDecimal.ROUND_FLOOR)).divide(pert,prec,BigDecimal.ROUND_HALF_UP);
            zD = BigDecimal.ZERO;
        ...
        // in the same time as it is copied it also makes the perturbation
        ...
        //Calculate the first 3 Points of the StackedPolytope.
        points = Algorithm.spDecimal.getPoints();
    ...
    private static void createPertPolytopDec(Face f, StackedPolytopeDec sP) {
        ...
        if (!f.isSmallestFace()) {
            ...
            x = (p.x().multiply(pert).setScale(0,BigDecimal.ROUND_FLOOR)).divide(pert,prec,BigDecimal.ROUND_HALF_UP);
            y = (p.y().multiply(pert).setScale(0,BigDecimal.ROUND_FLOOR)).divide(pert,prec,BigDecimal.ROUND_HALF_UP);
            z = BigDecimal.ZERO;
