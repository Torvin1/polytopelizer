    ...
    public CLI(String[] arguments) {
        ...
        if (arguments.length == 3 && arguments[0].equals("random")) {
            try {
                ...
                System.out
                        .println("<input file> <output file> - uses the given input file (.aN), calculates a stacked polytope and saves the result in the output file (.pol)");
                System.out
                ...
                System.out
                        .println("rate <n> <m> - this will compute the table [random <n'> <m>] for all 3 <= n'<= n");
            ...
            try {
                ...
                BigInteger maxY = BigInteger.ZERO;
                BigInteger maxZ = BigInteger.ZERO;
                BigInteger Counts = new BigInteger(String.valueOf(counts));
                for (int i = 3; i <= n; i++) {
                    ...
                    System.out.println(i + " " + maxX.divide(Counts) + " "
                            + maxY.divide(Counts) + " " + maxZ.divide(Counts));
