...
public class Algorithm {
    ...
    private static int prec = 18; // the precision of the number of decimals to
                                  // the division of a BigDecimal
    ...
    public static StackedPolytope calculateStackedPolytope1(ApollonianNetwork aN) {
        ...
        if (n < 4) {
            return new StackedPolytopeImpl(new PointInteger(),
                    new PointInteger(BigInteger.ONE, BigInteger.ZERO,
                            BigInteger.ZERO), new PointInteger(BigInteger.ZERO,
        ...
        for (int i = 0; i < 3; i++) {
            xD = ((new BigDecimal(px[i])).multiply(pert).setScale(0,
                    BigDecimal.ROUND_FLOOR)).divide(pert, prec,
                    BigDecimal.ROUND_HALF_UP);
            yD = ((new BigDecimal(py[i])).multiply(pert).setScale(0,
                    BigDecimal.ROUND_FLOOR)).divide(pert, prec,
                    BigDecimal.ROUND_HALF_UP);
            zD = BigDecimal.ZERO;
    ...
    private static void createPertPolytopDec(Face f, StackedPolytopeDec sP) {
        ...
        if (!f.isSmallestFace()) {
            ...
            // we need to calculate new coordinates from the balanced tree, so
            // that the areas from the flat polytope are equal
            ...
            // P = t1*A + t2*B + t3*C, where t1,t2,t3 are proportional to the
            // areas in opposition to the points A,B,C
            // in our case the formula is P = p0*f1 + p1*f2 + p2*f0, where f0 =
            // f0/s, f1 = f1/s, f2 = f2/s, s = f0 + f1 + f2
            ...
            x = p[0].x().multiply(f1).add(p[1].x().multiply(f2))
                    .add(p[2].x().multiply(f0));
            y = p[0].y().multiply(f1).add(p[1].y().multiply(f2))
                    .add(p[2].y().multiply(f0));

            // Make the rouding so that x and y are multiples of 1/pert
            x = (x.multiply(pert).setScale(0, BigDecimal.ROUND_FLOOR)).divide(
                    pert, prec, BigDecimal.ROUND_HALF_UP);
            y = (y.multiply(pert).setScale(0, BigDecimal.ROUND_FLOOR)).divide(
                    pert, prec, BigDecimal.ROUND_HALF_UP);
            z = BigDecimal.ZERO;
    ...
    private static void createAndScalePolytopInteger(StackedPolytopeDec sPdec,
            StackedPolytope sP) {
    ...
    private static void lift(StackedPolytopeDec sP) {
        ...
        if (!sP.isBoundary()) {
            ...
            // lift that point shift + actual_height and make it a multiple of
            // 1/pert_z
            PointDecimal p0 = new PointDecimal(p.x(), p.y(), new BigDecimal(
                    sP.vShift()).add(p.z()).multiply(pert_z)
                    .setScale(0, BigDecimal.ROUND_FLOOR)
                    .divide(pert_z, prec, BigDecimal.ROUND_HALF_UP));
    ...
    private static void changePoint(StackedPolytopeDec sP, PointDecimal point,
            PointDecimal p0) {
        int i;
        PointDecimal[] p = sP.getPoints();
